...
### Шифрование локальных DAG-ов

| Переменная | Обязательность | Назначение |
| --- | --- | --- |
| `DAG_FERNET_KEY` | Обязательна для `DagEncryptor.from_env()` | Ключ Fernet — 32 url-safe base64-байта. Несколько ключей перечисляются через запятую: шифрование идёт первым, расшифровка пробует все — так переживается ротация ключа без перезаливки уже загруженных объектов. |
| `DAGS_LOCAL_DIR` | Опциональна, по умолчанию `tests/dags` | Папка с локальными DAG-файлами, которые шифруются перед загрузкой в S3. |

Переменные читает только `DagEncryptor.from_env()`; конструктор принимает ключ и
папку явными аргументами. Ключ не попадает ни в логи, ни в тексты исключений,
ни в `repr()` — наружу отдаётся лишь количество ключей. Сгенерировать новый:
...
Скрытые HTTP-ретраи Kubernetes SDK отключены. Polling повторяет только временные
ответы `429`, `500`, `502`, `503`, `504` и временные сетевые ошибки, всегда в
пределах `OPENSHIFT_WAIT_TIMEOUT`. Ошибка проверки TLS-сертификата не является
временной: клиент прекращает ожидание после первого запроса и пишет событие
`openshift_wait_api_error_not_retried` с
`retry_reason="tls_certificate_validation"`. Для приватного CA следует задать
`OPENSHIFT_CA_CERT`; `OPENSHIFT_VERIFY_SSL=false` предназначена только для
явно выбранного небезопасного тестового окружения.
